...
--SELECT flight_ID,origin,destination,departuretime,departuredate,status FROM flights WHERE status = 'Cancelled';
...
-- Retrieve all flights that are from Manchester and delayed
-- SELECT flight_ID, origin,destination,departuretime,departuredate,status FROM flights WHERE origin = 'Manchester' AND status ='Delayed';

-- Retrieve all flights that are from Birmingham and to Spain
-- SELECT flight_ID, origin,destination,departuretime,departuredate,status FROM (SELECT * FROM flights JOIN destinations ON flights.destination=destinations.name WHERE country = 'Spain') WHERE origin = 'Birmingham';

-- Change a flight status to delayed
--UPDATE flights SET status = 'Delayed' WHERE flight_ID = 6;
--SELECT * FROM flights WHERE flight_ID = 6;

-- Change a departure time
--UPDATE flights SET departuretime = '12:30:00' WHERE flight_ID = 7;
--SELECT * FROM flights WHERE flight_ID = 7;
...
-- SELECT * FROM flights WHERE flight_ID = 16;
...
-- View flight destination codes outside England
-- SELECT DISTINCT destination_ID FROM (SELECT * FROM flights JOIN destinations ON flights.destination=destinations.name WHERE country <> 'England');
